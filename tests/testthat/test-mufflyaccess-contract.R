...
# cliff consumes the mufflyaccess measure x year x geography contract (v2.1.0).
# Values are the verified canonical cells from mufflyaccess 0.6.0; pathway
# sub-counts come from urps_count(), never from subtracting literals.
library(testthat)
...
test_that("cliff uses the canonical qualified 2023 active baseline", {
  ...
  expect_equal(abog_nat, 1031L)
  expect_equal(both_nat, 1332L)
...
test_that("2025 roster_snapshot is a DISTINCT cell (1339), not the 2023 active count", {
  ...
  expect_false(roster_2025 == active_2023)   # 1339 (roster) != 1332 (2023 active)
})
