...
|---|---|---|
| `demand-population.R` | ✅ done | Female population by single-year age × year (NPP SSOT); adds `birth_cohort` |
| `demand-birth_history.R` | ✅ done | Derives mean vaginal/cesarean deliveries per woman by birth cohort |
...
| `supply-staffing_conversion.R` | ✅ done | Service volumes → required FTE via CMS work RVUs + calibrated wrvu-per-FTE (ported from simulation R/17/R/23) |
| `08_scenarios.R` | ⬜ | Baseline, mode-of-delivery, reduced barriers, prevention, substitution |
| `validation-targets.R` | ✅ harness | Back-cast harness vs cited Nygaard 2008 + SWAN (Waetjen 2007) targets; full SWAN microdata validation stays in simulation legacy |

## Quick check

```r
source("demand_lifecourse/demand-population.R")
source("demand_lifecourse/demand-birth_history.R")
...
- Services→FTE conversion is now **CMS-work-RVU based** (`supply-staffing_conversion.R`,
